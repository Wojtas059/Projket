...
def modeSelection(): # Menu wyboru trybu pracy
    global MODE
    MENU_LIST = ['1', '2']
    MENU_OPTIONS = ['ONLINE MODE', 'DIAGNOSTIC MODE']
    MENU = set(MENU_LIST)        
    CHOICE = ''        
    while not CHOICE in MENU:
        print('OPERATING MODE SELECTION:')
        print('1. ONLINE MODE')        
        print('2. DIAGNOSTIC MODE')        
        CHOICE = input('CHOICE: ')
    MODE = MENU_OPTIONS[int(CHOICE) - 1]            
    print('\n*** SETTINGS ***')    
...
def dataOnline(): # Do implementacji w trybie online  
    # df['EMG'] <-- 400 probek sygnalu EMG
    # df['MMG'] <-- 400 probek sygnalu MMG
    # formatData()                
    # extractFeatures()                
    # rescaleFeatures()        
    # setColumns()                        
