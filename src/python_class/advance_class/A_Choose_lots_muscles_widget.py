...
class BattonLabelSpinner(RecycleDataViewBehavior, GridLayout):
    ...
    def get_values(self):
        ...
        if not content:
            ...
            self.data = [
                str(x[0]) + " " + str(x[1]) + " " + str(x[2]) for x in datalist
            ]
            ...
            if not instance.findAnyEmail(self.email_add_user.text):
                if instance.findUserPrivilegesById(
                    ...
                    if instance.insertUserAndAdvanced(
                        self.id_, self.email_add_user.text
                    ):
...
class AChooseLotsMusclesWidget(Screen):
    ...
    def choose_clicked(self, many):
        ...
        self.recyView.data = []
        self.recyView.data = [
            {"text": str(x + 1), "values": str(self.id_)} for x in range(int(many))
        ]
    ...
    def on_click(self, many):
        try:
            ...
            self.error_pop(
                "Error",
                "Aby przejść dalej musisz wybrać ilość mięśni,\n a także jakie to będą mieśnie",
            )
            return False
