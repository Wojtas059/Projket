...
from src.proto_file.ExampleProto_pb2 import DataPacket, DeviceRequest, DeviceResponse
from src.proto_file.proto_parser import ProtobufCommParser
...
class ProtobufComm:
    def __init__(self, port: str, baudrate: int):
        ...
        if not self.stm.is_open:
            raise RuntimeError(f"Couldn't open port {port} with baudrate {baudrate}")
    ...
    def handle_response(self, response: DeviceResponse):
        if response.code == DeviceResponse.ResponseType.OK:
            ...
            print(
                "Some protobuf error happened while parsing the request on the device!"
            )
    ...
    def handle_data(self):
        ...
        logging.info(
            f"{data.valueA}, {data.valueB}, {data.valueC}, {data.constantValue}"
        )
        return str(
            f"{data.valueA}, {data.valueB}, {data.valueC}, {data.constantValue}\n"
        )
