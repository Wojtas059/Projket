...
// Use this to check if S2-LP is in correct state.
// If the state is not one of the states from S2LP_State enum,
// something is very wrong and you have to take care of it.
bool S2LP_IsInCorrectState(S2LP_Handle* handle);
...
void S2LP_SetExtRef(S2LP_Handle* handle, bool state);
